...
int exynos_usb_audio_map_buf(struct usb_device *udev)
{
	...
	if (ret < 0) {
		pr_err("abox iommu un-mapping for in buf is failed %d\n", ret);
		return ret;
	}
	...
	if (ret < 0) {
		pr_err("abox iommu unmapping for pcm out buf is failed\n");
		return ret;
	}
...
int exynos_usb_audio_setintf(struct usb_device *udev, int iface, int alt, int direction)
{
	...
	if (direction) {
		...
		if (!usb_audio->indeq_map_done || (hwinfo->in_deq != hwinfo->old_in_deq)) {
			...
			ret = abox_iommu_map(dev, USB_AUDIO_IN_DEQ,
					...
					PAGE_SIZE, 0);
			if (ret < 0) {
				pr_err("abox iommu mapping for in buf is failed %d\n", ret);
				goto err;
			}
		}
		...
		if (hwinfo->fb_out_deq) {
			...
			if (!usb_audio->fb_outdeq_map_done || (hwinfo->fb_out_deq != hwinfo->fb_old_out_deq)) {
				usb_audio->fb_outdeq_map_done = 1;
				offset = hwinfo->fb_old_out_deq % PAGE_SIZE;
				ret = abox_iommu_unmap(dev, USB_AUDIO_FBOUT_DEQ);
				if (ret < 0) {
					pr_err("abox iommu un-mapping for fb_out buf is failed %d\n", ret);
					goto err;
				}
				...
				ret = abox_iommu_map(dev, USB_AUDIO_FBOUT_DEQ,
						...
						PAGE_SIZE, 0);
				if (ret < 0) {
					pr_err("abox iommu mapping for fb_out buf is failed %d\n", ret);
					goto err;
				}
			}
		}
		...
		if (!usb_audio->outdeq_map_done || (hwinfo->out_deq != hwinfo->old_out_deq)) {
			...
			if (ret < 0) {
				pr_err("abox iommu un-mapping for out buf is failed %d\n", ret);
				goto err;
			}
			...
			ret = abox_iommu_map(dev, USB_AUDIO_OUT_DEQ,
					...
					PAGE_SIZE, 0);
			if (ret < 0) {
				pr_err("abox iommu mapping for out buf is failed %d\n", ret);
				goto err;
			}
		}
		...
		if (hwinfo->fb_in_deq) {
			...
			if (!usb_audio->fb_indeq_map_done || (hwinfo->fb_in_deq != hwinfo->fb_old_in_deq)) {
				usb_audio->fb_indeq_map_done = 1;
			offset = hwinfo->fb_old_in_deq % PAGE_SIZE;
			ret = abox_iommu_unmap(dev, USB_AUDIO_FBIN_DEQ);
			if (ret < 0) {
				pr_err("abox iommu un-mapping for fb_in buf is failed %d\n", ret);
				goto err;
			}
			...
			ret = abox_iommu_map(dev, USB_AUDIO_FBIN_DEQ,
					...
					PAGE_SIZE, 0);
			if (ret < 0) {
				pr_err("abox iommu mapping for fb_in buf is failed %d\n", ret);
				goto err;
			}
		}
		}
...
int exynos_usb_audio_hcd(struct usb_device *udev)
{
	...
	ret = abox_iommu_map(dev, USB_AUDIO_XHCI_BASE, USB_AUDIO_XHCI_BASE, PAGE_SIZE, 0);
	if (ret) {
