...
#define S2MU106_USBPD_DEV_NAME    "usbpd-s2mu106"
...
#define S2MU106_REG_SEND_EN_CLEAR_SHIFT  	   	(7)
#define S2MU106_REG_RD_OR_VBUS_MUX_SEL    \
        (0x1 << S2MU106_REG_RD_OR_VBUS_MUX_SEL_SHIFT)
#define S2MU106_REG_SEND_EN_CLEAR    \
        (0x1 << S2MU106_REG_SEND_EN_CLEAR_SHIFT)
...
//    struct s2m_pdic_notifier_struct pdic_notifier;
	...
	struct delayed_work s2mu106_poll_check;
#endif
    int pm_cc1;
    int pm_cc2;
    int pm_chgin;
...
extern int sec_pps_enable(int num, int ppsVol, int ppsCur);
extern int sec_get_pps_voltage(void);
#endif
extern void pdo_ctrl_by_flash(bool mode);
#endif
#if defined(CONFIG_DUAL_ROLE_USB_INTF)
extern void s2mu106_role_swap_check(struct work_struct *wk);
extern int s2mu106_dual_role_is_writeable(struct dual_role_phy_instance *drp,
				  enum dual_role_property prop);
extern int s2mu106_dual_role_get_local_prop(struct dual_role_phy_instance *dual_role,
				    enum dual_role_property prop,
				    unsigned int *val);
extern int s2mu106_dual_role_set_prop(struct dual_role_phy_instance *dual_role,
			      enum dual_role_property prop,
			      const unsigned int *val);
extern int s2mu106_dual_role_init(void *_data);
#elif defined(CONFIG_TYPEC)
extern void s2mu106_typec_role_swap_check(struct work_struct *wk);
extern int s2mu106_typec_port_type_set(const struct typec_capability *cap,
					enum typec_port_type port_type);
extern int s2mu106_typec_get_pd_support(void *_data);
extern int s2mu106_typec_init(void *_data);
#endif
