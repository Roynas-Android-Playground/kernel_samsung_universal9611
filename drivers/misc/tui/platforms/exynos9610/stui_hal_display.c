...
int stui_alloc_video_space(struct device *dev, struct tui_hw_buffer *buffer)
{
	...
	framebuf_size = STUI_ALIGN_UP(framebuf_size, STUI_ALIGN_8kB_SZ);
	workbuf_size = (lcd_info->xres * lcd_info->yres * (2 * (DEFAULT_BPP >> 3) + 1));
	workbuf_size = STUI_ALIGN_UP(workbuf_size, STUI_ALIGN_8kB_SZ);

	g_dma_buf = ion_alloc_dmabuf("vstream_heap",
					framebuf_size + workbuf_size + STUI_ALIGN_8kB_SZ,
					ION_FLAG_PROTECTED);
	if (IS_ERR(g_dma_buf)) {
	...
	phys_addr = STUI_ALIGN_UP(phys_addr, STUI_ALIGN_8kB_SZ);
