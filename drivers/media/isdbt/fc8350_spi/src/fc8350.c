...
static struct isdbt_platform_data *isdbt_populate_dt_pdata(struct device *dev)
{
	...
	/* SPI device node */
	pdata->spi_device_node = of_parse_phandle(dev->of_node, "isdbt,spi-node", 0);
	if (!IS_ERR_OR_NULL(pdata->spi_device_node)) {
		pdata->spi_pdev = of_find_device_by_node(pdata->spi_device_node);
		if (!pdata->spi_pdev){
			pr_err("%s : unable to find spi device\n", __func__);
			goto alloc_err;
		}
	}
	else {
		pr_err("%s : can not find the isdbt-handler-property spi-node in the dt\n"
			, __func__);
	}
...
static int isdbt_probe(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(isdbt_pdata->gpio_dtv_check)) {
		if (gpio_get_value(isdbt_pdata->gpio_dtv_check)) {
		    ...
		    /* OneBinary concept: if non-dtv device, put SPI in NC mode */
			if (!IS_ERR_OR_NULL(isdbt_pdata->spi_device_node)) {
				isdbt_pinctrl = devm_pinctrl_get_select(&isdbt_pdata->spi_pdev->dev, "isdbt_spi_nc");
				if (IS_ERR(isdbt_pinctrl))
						pr_err("%s : Failed to configure isdbt NC pin\n", __func__);
				else{
					pr_info("%s : configured isdbt SPI in NC mode\n", __func__);
					devm_pinctrl_put(isdbt_pinctrl);
				}
			}
