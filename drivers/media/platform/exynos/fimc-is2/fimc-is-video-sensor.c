	...
	struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_IS_S_TUNING_CONFIG:
		...
		break;
#endif
	case V4L2_CID_IS_S_FD_MODE:
		if (device->ischain) {
			device->ischain->use_vra_nfd = (u32)ctrl->value;
			info("%s: Use FD HW (0x%x)\n", __func__, device->ischain->use_vra_nfd);
		} else {
			err("%s: ischain is not connected (id: 0x%08X)\n", __func__, ctrl->id);
		}
		break;
	default:
