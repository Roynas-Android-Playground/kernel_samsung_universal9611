...
int fimc_is_eeprom_parse_dt(struct device_node *dnode, int rom_id)
{
	int ret = 0;
	...
	const struct i2c_device_id *id)
{
	...
	if (id->driver_data >= SENSOR_POSITION_REAR && id->driver_data < SENSOR_POSITION_MAX) {
	...
	if (client->dev.of_node) {
		if(fimc_is_eeprom_parse_dt(client->dev.of_node, device->driver_data)) {
