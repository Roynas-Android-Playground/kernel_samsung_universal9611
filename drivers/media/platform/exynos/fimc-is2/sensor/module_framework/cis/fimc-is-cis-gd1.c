...
#include "fimc-is-cis-gd1.h"
#include "fimc-is-cis-gd1-setA.h"
#include "fimc-is-cis-gd1-setB.h"
...
static bool sensor_gd1_cal_write_flag;

#ifdef CONFIG_VENDER_MCD_V2
extern struct fimc_is_lib_support gPtr_lib_support;
#endif

static void sensor_gd1_set_integration_max_margin(u32 mode, cis_shared_data *cis_data)
{
...
static void sensor_gd1_set_integration_min(u32 mode, cis_shared_data *cis_data)
{
	FIMC_BUG_VOID(!cis_data);
...
#ifdef USE_AP_PDAF
static void sensor_gd1_cis_set_paf_stat_enable(u32 mode, cis_shared_data *cis_data)
{
	WARN_ON(!cis_data);

	switch (mode) {
	case SENSOR_GD1_2X2BIN_3264X2448_30FPS:
	case SENSOR_GD1_2X2BIN_3264X1836_30FPS:
	case SENSOR_GD1_2X2BIN_2640X1980_30FPS:
	case SENSOR_GD1_2X2BIN_2640X1488_30FPS:
...
#if SENSOR_GD1_DEBUG_INFO
static int sensor_gd1_cis_fine_integration_value(struct fimc_is_cis *cis)
{
	int ret = 0;
	u8 fine_integ_min = 0;
	...
	ret = fimc_is_sensor_write16(client, SENSOR_GD1_DIRECT_PAGE_ACCESS_MARK, SENSOR_GD1_DIRECT_DEFALT_PAGE_ADDR);
	if (ret < 0) {
		err("i2c fail addr(%x, %x) ,ret = %d\n",
			SENSOR_GD1_DIRECT_PAGE_ACCESS_MARK, SENSOR_GD1_DIRECT_DEFALT_PAGE_ADDR, ret);
		goto i2c_err;
	}
...
static void sensor_gd1_cis_data_calculation(const struct sensor_pll_info_compact *pll_info, cis_shared_data *cis_data)
{
	u32 total_pixels = 0;
	u32 pixel_rate = 0;
	...
	cis_data->max_margin_fine_integration_time = SENSOR_GD1_FINE_INTEGRATION_TIME_MAX_MARGIN;
...
static int sensor_gd1_wait_stream_off_status(cis_shared_data *cis_data)
{
	int ret = 0;
	u32 timeout = 0;
...
int sensor_gd1_cis_check_rev(struct fimc_is_cis *cis)
{
	int ret = 0;
	u8 rev8 = 0;
	u8 data1 = 0;
	...
	ret = fimc_is_sensor_read8(client, SENSOR_GD1_REVISION_NUM_ADDR, &rev8);
	if (ret < 0) {
		err("is_sensor_read is fail, (ret %d)", ret);
		ret = -EAGAIN;
		goto i2c_err;
...
int sensor_gd1_cis_check_rev_on_init(struct v4l2_subdev *subdev)
{
#if 0
	int ret = 0;
	struct i2c_client *client;
	...
	ret = sensor_gd1_cis_check_rev(cis);
...
#if SENSOR_GD1_CAL_DEBUG
int sensor_gd1_cis_cal_dump(char* name, char *buf, size_t size)
{
	int ret = 0;
...
int sensor_gd1_cis_XTCCal_write(struct v4l2_subdev *subdev)
{
	int ret = 0;
	...
	int position;
	ulong cal_addr;
	u8 cal_data[SENSOR_GD1_XTC_CAL_SIZE] = {0, };
	...
	if (position == SENSOR_POSITION_REAR) {
		cal_addr += SENSOR_GD1_XTC_CAL_BASE_REAR;
	} else {
		err("cis_gd1 position(%d) is invalid!\n", position);
		goto p_err;
	}
#else
	if (position == SENSOR_POSITION_REAR){
		cal_addr = lib->minfo->kvaddr_cal[position] + SENSOR_GD1_XTC_CAL_BASE_REAR;
	}else {
		err("cis_gd1 position(%d) is invalid!\n", position);
		goto p_err;
	}
	...
	memcpy(cal_data, (u16 *)cal_addr, SENSOR_GD1_XTC_CAL_SIZE);

#if SENSOR_GD1_CAL_DEBUG
	ret = sensor_gd1_cis_cal_dump(SENSOR_GD1_XTC_DUMP_NAME, (char *)cal_data, (size_t)SENSOR_GD1_XTC_CAL_SIZE);
	if (ret < 0) {
		err("cis_gd1 XTC Cal dump fail(%d)!\n", ret);
	}
#endif
	...
	/* page select */
	ret = fimc_is_sensor_write16(client, SENSOR_GD1_INDIRECT_WRITE_PAGE_ADDR, SENSOR_GD1_XTC_PAGE_ADDR);
	if (ret < 0) {
		goto i2c_err;
	}
	/* write register address ( in auto increment reg) */
	ret = fimc_is_sensor_write16(client, SENSOR_GD1_INDIRECT_WRITE_OFFSET_ADDR, SENSOR_GD1_XTC_REG_ADDR);
	if (ret < 0) {
		goto i2c_err;
	}
	/* write data in write register */
	ret = fimc_is_sensor_write8_sequential(client, SENSOR_GD1_INDIRECT_WRITE_DATA_ADDR, cal_data, SENSOR_GD1_XTC_CAL_SIZE);
	if (ret < 0) {
		goto i2c_err;
	}
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
	...
	ret = sensor_cis_set_registers(subdev, sensor_gd1_xtc_additional_setting, ARRAY_SIZE(sensor_gd1_xtc_additional_setting));
	if (ret < 0) {
...
/* CIS OPS */
int sensor_gd1_cis_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	u32 setfile_index = 0;
...
#endif
		warn("sensor_gd1_check_rev is fail when cis init");
		cis->rev_flag = true;
	...
	***********************************************************************/
	sensor_gd1_cal_write_flag = false;
	...
	sensor_gd1_set_integration_max_margin(setfile_index, cis->cis_data);
	sensor_gd1_set_integration_min(setfile_index, cis->cis_data);
...
int sensor_gd1_cis_dump_registers(struct v4l2_subdev *subdev, const u32 *regs, const u32 size)
{
	int ret = 0;
	int i = 0;
	struct fimc_is_cis *cis;
	...
	for (i = 0; i < size; i += I2C_NEXT) {
		if (regs[i + I2C_BYTE] == 0x2 && regs[i + I2C_ADDR] == 0x6028) {
			...
			ret = fimc_is_sensor_write16(client, SENSOR_GD1_INDIRECT_READ_PAGE_ADDR,  regs[i + I2C_DATA]);
		}
		if (regs[i + I2C_BYTE] == 0x2 && regs[i + I2C_ADDR] == 0x602A) {
			pr_err("[SEN:DUMP WRITE] [0x%04X, 0x%04X\n", regs[i + I2C_ADDR], regs[i + I2C_DATA]);
			 ret = fimc_is_sensor_write16(client, SENSOR_GD1_INDIRECT_READ_OFFSET_ADDR, regs[i + I2C_DATA]);
		}
...
int sensor_gd1_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client = NULL;
	...
	pr_info("[%s] *******************************\n", __func__);
	ret = fimc_is_sensor_read16(client, SENSOR_GD1_MODEL_ID_ADDR, &data16);
	if (unlikely(!ret)) pr_info("[SEN:INFO]model_id(0x%x)\n", data16);
	else goto i2c_err;
	ret = fimc_is_sensor_read8(client, SENSOR_GD1_REVISION_NUM_ADDR, &data8);
	if (unlikely(!ret)) pr_info("[SEN:INFO]revision_number(0x%x)\n", data8);
	else goto i2c_err;
	ret = fimc_is_sensor_read8(client, SENSOR_GD1_FRAME_COUNT_ADDR, &data8);
	if (unlikely(!ret)) pr_info("[SEN:INFO]frame_count(0x%x)\n", data8);
	else goto i2c_err;
	ret = fimc_is_sensor_read8(client, SENSOR_GD1_SETUP_MODE_SELECT_ADDR, &data8);
	if (unlikely(!ret)) pr_info("[SEN:INFO]setup_mode(streaming)(0x%x)\n", data8);
	else goto i2c_err;
	ret = fimc_is_sensor_read16(client, SENSOR_GD1_EXTCLK_FREQ_ADDR, &data16);
	if (unlikely(!ret)) pr_info("[SEN:INFO]extclk_feq_mhz(0x%x)\n", data16);
	else goto i2c_err;
	ret = fimc_is_sensor_read16(client, SENSOR_GD1_CORASE_INTEG_TIME_ADDR, &data16);
	if (unlikely(!ret)) pr_info("[SEN:INFO]coarse_integration_time(0x%x)\n", data16);
	else goto i2c_err;
	ret = fimc_is_sensor_read16(client, SENSOR_GD1_AGAIN_CODE_GLOBAL_ADDR, &data16);
	if (unlikely(!ret)) pr_info("[SEN:INFO]gain_code_global(0x%x)\n", data16);
	else goto i2c_err;
	ret = fimc_is_sensor_read16(client, SENSOR_GD1_FRAME_LENGTH_LINE_ADDR, &data16);
	if (unlikely(!ret)) pr_info("[SEN:INFO]frame_length_line(0x%x)\n", data16);
	else goto i2c_err;
	ret = fimc_is_sensor_read16(client, SENSOR_GD1_LINE_LENGTH_PCK_ADDR, &data16);
	if (unlikely(!ret)) pr_info("[SEN:INFO]line_length_pck(0x%x)\n", data8);
	else goto i2c_err;

	pr_info("[%s] *************Mode settings dump start******************\n", __func__);
	 sensor_gd1_cis_dump_registers(subdev, sensor_gd1_setfiles[0], sensor_gd1_setfile_sizes[0]);
...
static int sensor_gd1_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = fimc_is_sensor_write8(client, SENSOR_GD1_GROUP_PARAM_HOLD_ADDR, hold);
	if (ret < 0)
		goto i2c_err;
...
int sensor_gd1_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
...
int sensor_gd1_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
	...
	/* If check_rev of S5KGD1 fail when cis_init, one more check_rev in mode_change */
	if (cis->rev_flag == true) {
		cis->rev_flag = false;
		ret = sensor_gd1_cis_check_rev(cis);
		if (ret < 0) {
			err("sensor_gd1_check_rev is fail");
			goto p_err;
		}
	}

#if SENSOR_GD1_WRITE_XTC_CAL
	if (IS_REMOSAIC(mode) && sensor_gd1_cal_write_flag == false) {
		sensor_gd1_cal_write_flag = true;

		info("[%s] mode is XTC Remosaic Mode! Write XTC data.\n", __func__);

		ret = sensor_gd1_cis_XTCCal_write(subdev);
		if (ret < 0) {
			err("sensor_gd1_XTC_Cal_write fail!! (%d)", ret);
			goto p_err;
		}
	}
#endif
#ifdef USE_AP_PDAF
	sensor_gd1_cis_set_paf_stat_enable(mode, cis->cis_data);
#endif
	I2C_MUTEX_LOCK(cis->i2c_lock);
...
#if SENSOR_GD1_DEBUG_INFO
	sensor_gd1_cis_fine_integration_value(cis);
#endif
...
int sensor_gd1_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	int ret = 0;
	bool binning = false;
	...
	ret = sensor_gd1_wait_stream_off_status(cis_data);
	if (ret) {
		err("Must stream off\n");
		ret = -EINVAL;
		goto p_err;
	...
	if (binning) {
		ratio_w = (SENSOR_GD1_MAX_WIDTH / cis_data->cur_width);
		ratio_h = (SENSOR_GD1_MAX_HEIGHT / cis_data->cur_height);
	} else {
		ratio_w = 1;
		ratio_h = 1;
	}

	if (((cis_data->cur_width * ratio_w) > SENSOR_GD1_MAX_WIDTH) ||
		((cis_data->cur_height * ratio_h) > SENSOR_GD1_MAX_HEIGHT)) {
	...
	start_x = ((SENSOR_GD1_MAX_WIDTH - cis_data->cur_width * ratio_w) / 2) & (~0x1);
	start_y = ((SENSOR_GD1_MAX_HEIGHT - cis_data->cur_height * ratio_h) / 2) & (~0x1);
...
int sensor_gd1_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
	...
	sensor_gd1_cis_group_param_hold(subdev, 0x01);
	if (ret < 0)
		err("group_param_hold_func failed at stream on");

	I2C_MUTEX_LOCK(cis->i2c_lock);

#ifdef SENSOR_GD1_DEBUG_INFO
	{
	u16 pll;
	...
	/* Sensor stream on */
	ret = fimc_is_sensor_write16(client, SENSOR_GD1_SETUP_MODE_SELECT_ADDR, 0x0100);
	if (ret < 0)
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x0100, 0x0100, ret);

	I2C_MUTEX_UNLOCK(cis->i2c_lock);

	sensor_gd1_cis_group_param_hold(subdev, 0x00);
	if (ret < 0)
		err("group_param_hold_func failed at stream on");

#if 0 //SENSOR_GD1_DEBUG_INFO
...
int sensor_gd1_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
	...
	/* Sensor stream off */
	ret = fimc_is_sensor_write16(client, SENSOR_GD1_SETUP_MODE_SELECT_ADDR, 0x0000);
	if (ret < 0)
		err("i2c transfer fail addr(%x), val(%x), ret = %d\n", 0x0100, 0x0000, ret);
...
int sensor_gd1_cis_adjust_frame_duration(struct v4l2_subdev *subdev,
...
int sensor_gd1_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	ret = fimc_is_sensor_write16(client, SENSOR_GD1_FRAME_LENGTH_LINE_ADDR, frame_length_lines);
	if (ret < 0)
		goto i2c_err;
...
int sensor_gd1_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_gd1_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	/* Short exposure */
	ret = fimc_is_sensor_write16(client, SENSOR_GD1_CORASE_INTEG_TIME_ADDR, short_coarse_int);
	if (ret < 0)
		goto i2c_err;
...
int sensor_gd1_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
...
int sensor_gd1_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_gd1_cis_adjust_analog_gain(struct v4l2_subdev *subdev, u32 input_again, u32 *target_permile)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_gd1_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	ret = fimc_is_sensor_write16(client, SENSOR_GD1_AGAIN_CODE_GLOBAL_ADDR, analog_gain);
	if (ret < 0)
		goto i2c_err;
...
int sensor_gd1_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	ret = fimc_is_sensor_read16(client, SENSOR_GD1_AGAIN_CODE_GLOBAL_ADDR, &analog_gain);
	if (ret < 0)
		goto i2c_err;
...
int sensor_gd1_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_gd1_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_gd1_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	/* Short digital gain */
	ret = fimc_is_sensor_write16(client, SENSOR_GD1_DGAIN_GLOBAL_ADDR, short_dgain);
	if (ret < 0)
		goto i2c_err;
...
int sensor_gd1_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	ret = fimc_is_sensor_read16(client, SENSOR_GD1_DGAIN_GLOBAL_ADDR, &digital_gain);
	if (ret < 0)
		goto i2c_err;
...
int sensor_gd1_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_gd1_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
void sensor_gd1_cis_data_calc(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (cis->rev_flag == true) {
		...
		ret = sensor_gd1_cis_check_rev(cis);
		if (ret < 0) {
			err("sensor_gd1_check_rev is fail: ret(%d)", ret);
			return;
		}
	}
...
int sensor_gd1_cis_set_adjust_sync(struct v4l2_subdev *subdev, u32 sync)
{
	int ret = 0;
...
int sensor_gd1_cis_set_wb_gain(struct v4l2_subdev *subdev, struct wb_gains wb_gains)
{
#define ABS_WB_GAIN_NUM	3
	...
	hold = sensor_gd1_cis_group_param_hold(subdev, 0x01);
	if (hold < 0) {
		ret = hold;
		goto p_err;
	}
	...
	for (index=0; index < ABS_WB_GAIN_NUM; index++) {
		ret = fimc_is_sensor_write16(client, (SENSOR_GD1_WBGAIN_RED + (index * 2)), abs_gains[index]);
		if (ret < 0) {
			err("i2c fail addr(%x), val(%04x), ret = %d\n",
				SENSOR_GD1_WBGAIN_RED + (index * 2), abs_gains[index], ret);
			goto i2c_err;
		}
	}
...
p_err:
	if (hold > 0) {
		hold = sensor_gd1_cis_group_param_hold(subdev, 0x00);
		if (hold < 0)
			ret = hold;
	}
...
int sensor_gd1_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
static struct fimc_is_cis_ops cis_ops_gd1 = {
	...
	.cis_wait_streamoff = sensor_gd1_cis_wait_streamoff,
	.cis_wait_streamon = sensor_cis_wait_streamon,
	.cis_data_calculation = sensor_gd1_cis_data_calc,
	.cis_set_adjust_sync = sensor_gd1_cis_set_adjust_sync,
	.cis_check_rev = sensor_gd1_cis_check_rev_on_init,
	.cis_set_initial_exposure = sensor_cis_set_initial_exposure,
	.cis_set_wb_gains = sensor_gd1_cis_set_wb_gain,
};

int cis_gd1_probe(struct i2c_client *client,
...
static const struct of_device_id sensor_cis_gd1_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-gd1",
	},
	{},
};
