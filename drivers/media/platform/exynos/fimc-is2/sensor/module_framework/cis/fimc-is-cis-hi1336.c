...
#include "fimc-is-cis-hi1336-12M-full-setA.h"
#include "fimc-is-cis-hi1336-12M-full-setB.h"
#elif defined(USE_HI1336B_SETFILE)
#include "fimc-is-cis-hi1336B-setA.h"
#include "fimc-is-cis-hi1336B-setB.h"
#else
...
int sensor_hi1336_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again, struct ae_param *dgain)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis   = NULL;
	...
	struct ae_param *again, struct ae_param *dgain)
{
	...
	dbg_sensor(1, "[%s] sensor_hi1336_cis_set_totalgain..!!\n", __func__);

	/* Set Exposure Time */
	ret = sensor_hi1336_cis_set_exposure_time(subdev, target_exposure);
	if (ret < 0) {
		err("[%s] sensor_hi1336_cis_set_exposure_time fail\n", __func__);
		goto p_err;
	}

	/* Set Analog & Digital gains */
	ret = sensor_hi1336_cis_set_analog_gain(subdev, again, dgain);
	if (ret < 0) {
		err("[%s] sensor_hi1336_cis_set_analog_gain fail\n", __func__);
		goto p_err;
	}
