...
#include "fimc-is-actuator-dw9817.h"
...
#define DEF_DW9817_FIRST_POSITION		512
#define DEF_DW9817_FIRST_DELAY			20

#define DEF_DW9817_OFFSET_POSITION_MAX 60
#define DEF_DW9817_AF_PAN 450
#define DEF_DW9817_PRESET_MAX 255

extern struct fimc_is_lib_support gPtr_lib_support;
extern struct fimc_is_sysfs_actuator sysfs_actuator;

int sensor_dw9817_init(struct i2c_client *client, struct fimc_is_caldata_list_dw9817 *cal_data)
{
	int ret = 0;
	u8 i2c_data[2];
	...
	if (!cal_data) {
		...
		/* wait 5ms after power-on for DW9817 */
...
static int sensor_dw9817_write_position(struct i2c_client *client, u32 val)
{
	int ret = 0;
	...
	if (val > DW9817_POS_MAX_SIZE) {
		err("Invalid af position(position : %d, Max : %d).\n",
					val, DW9817_POS_MAX_SIZE);
		ret = -EINVAL;
		goto p_err;
	}
...
/*
static int sensor_dw9817_valid_check(struct i2c_client * client)
{
	int i;
...
static void sensor_dw9817_print_log(int step)
{
	int i;
...
*/
static int sensor_dw9817_init_position(struct i2c_client *client,
		struct fimc_is_actuator *actuator)
{
	int ret = 0;
	ret = sensor_dw9817_write_position(client, DEF_DW9817_FIRST_POSITION);
	if (ret < 0)
		goto p_err;

	msleep(DEF_DW9817_FIRST_DELAY);

	actuator->position = DEF_DW9817_FIRST_POSITION;

	dbg_actuator("initial position %d, setting\n",
			DEF_DW9817_FIRST_POSITION);

p_err:
	return ret;
}

int sensor_dw9817_actuator_init(struct v4l2_subdev *subdev, u32 val)
{
	int ret = 0;
	struct fimc_is_actuator *actuator;
	struct fimc_is_caldata_list_dw9817 *cal_data = NULL;
	...
	cal_data = (struct fimc_is_caldata_list_dw9817 *)(cal_addr);

	/* Read into EEPROM data or default setting */
	ret = sensor_dw9817_init(client, cal_data);
	if (ret < 0){
#ifdef USE_CAMERA_HW_BIG_DATA
	...
	ret = sensor_dw9817_init_position(client, actuator);
	if (ret < 0)
		goto p_err;
...
int sensor_dw9817_actuator_get_status(struct v4l2_subdev *subdev, u32 *info)
{
	int ret = 0;
	u8 val = 0;
...
int sensor_dw9817_actuator_wait_busy(struct v4l2_subdev *subdev)
{
	u32 info;
	int count = 0;
	msleep(5);
	do {
		sensor_dw9817_actuator_get_status(subdev, &info);
...
int sensor_dw9817_actuator_soft_landing(struct v4l2_subdev *subdev)
{
	int ret = 0;
	u8 val = 0;
	int interval = 0;
	...
	sensor_dw9817_actuator_wait_busy(subdev);
	...
	interval = (DEF_DW9817_FIRST_POSITION - position) / 3;

	for(i = 0; i < 3; i++)
	{
		position += interval;
		ret = sensor_dw9817_write_position(client, position);
		if (ret < 0)
			goto p_err;
		sensor_dw9817_actuator_wait_busy(subdev);
	}

	pr_info("[%s] Softlanding Successful, final position: [%x]\n",__func__, DEF_DW9817_FIRST_POSITION);
	return ret;	
p_err:
...
#endif
int sensor_dw9817_actuator_set_position(struct v4l2_subdev *subdev, u32 *info)
{
	int ret = 0;
	struct fimc_is_actuator *actuator;
	struct i2c_client *client;
	...
	position = *info;
	if (position > DW9817_POS_MAX_SIZE) {
		err("Invalid af position(position : %d, Max : %d).\n",
					position, DW9817_POS_MAX_SIZE);
		ret = -EINVAL;
		goto p_err;
	}
...
static int sensor_dw9817_actuator_g_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	int ret = 0;
	u32 val = 0;
	...
	case V4L2_CID_ACTUATOR_GET_STATUS:
		ret = sensor_dw9817_actuator_get_status(subdev, &val);
		if (ret < 0) {
...
static int sensor_dw9817_actuator_s_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	int ret = 0;

	switch(ctrl->id) {
	case V4L2_CID_ACTUATOR_SET_POSITION:
		ret = sensor_dw9817_actuator_set_position(subdev, &ctrl->value);
		if (ret) {
	...
	case V4L2_CID_ACTUATOR_SOFT_LANDING:
		ret = sensor_dw9817_actuator_soft_landing(subdev);
...
static const struct v4l2_subdev_core_ops core_ops = {
	.init = sensor_dw9817_actuator_init,
	.g_ctrl = sensor_dw9817_actuator_g_ctrl,
	.s_ctrl = sensor_dw9817_actuator_s_ctrl,
};
...
int sensor_dw9817_actuator_probe(struct i2c_client *client,
		const struct i2c_device_id *id)
{
	...
	actuator->id = ACTUATOR_NAME_DW9817;
	actuator->subdev = subdev_actuator;
		actuator->device = sensor_id[i];
	actuator->client = client;
	actuator->position = 0;
	actuator->max_position = DW9817_POS_MAX_SIZE;
	actuator->pos_size_bit = DW9817_POS_SIZE_BIT;
	actuator->pos_direction = DW9817_POS_DIRECTION;
...
static int sensor_dw9817_actuator_remove(struct i2c_client *client)
{
	int ret = 0;
...
static const struct of_device_id exynos_is_dw9817_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-actuator-dw9817",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_is_dw9817_match);

static const struct i2c_device_id actuator_dw9817_idt[] = {
	{ ACTUATOR_NAME, 0 },
	{},
};

static struct i2c_driver actuator_dw9817_driver = {
	.driver = {
		.name	= ACTUATOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_is_dw9817_match
	},
	.probe	= sensor_dw9817_actuator_probe,
	.remove	= sensor_dw9817_actuator_remove,
	.id_table = actuator_dw9817_idt
};
module_i2c_driver(actuator_dw9817_driver);
