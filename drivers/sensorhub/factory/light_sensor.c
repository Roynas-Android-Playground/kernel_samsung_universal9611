...
#ifdef CONFIG_SENSORS_SSP_LIGHT_DUAL
static ssize_t light_factory_fstate_store(struct device *dev,
					  struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = ssp_send_command(data, CMD_SETVALUE, SENSOR_TYPE_LIGHT,
			       LIGHT_FACTORY_STATE, 0, (char *)&temp, sizeof(temp), NULL, NULL);

	if (ret != SUCCESS) {
		ssp_err("LIGHT_FACTORY_STATE CMD fail %d", ret);
		return -EINVAL;
	}
...
static DEVICE_ATTR(fac_fstate, S_IWUSR | S_IWGRP, NULL, light_factory_fstate_store);
#endif
