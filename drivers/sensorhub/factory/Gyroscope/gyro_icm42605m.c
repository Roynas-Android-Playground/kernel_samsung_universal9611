...
ssize_t get_gyro_icm42605m_name(char *buf)
{
	return sprintf(buf, "%s\n", "ICM42605M");
}

ssize_t get_gyro_icm42605m_vendor(char *buf)
{
	return sprintf(buf, "%s\n", "TDK");
}

ssize_t get_gyro_icm42605m_power_off(char *buf)
{
	ssp_infof();
...
ssize_t get_gyro_icm42605m_power_on(char *buf)
{
	ssp_infof();
...
ssize_t get_gyro_icm42605m_temperature(struct ssp_data *data, char *buf)
{
	char *buffer = NULL;
	int buffer_length = 0;
...
u32 icm42605m_selftest_sqrt(u32 sqsum)
{
	u32 sq_rt;
	u32 g0, g1, g2, g3, g4;
	u32 seed;
...
ssize_t get_gyro_icm42605m_selftest(struct ssp_data *data, char *buf)
{
	char *temp_buf = NULL;
	...
	/* bias */
	gyro_self_bias[0]
	        = st_bias[0] * DEF_GYRO_SENS_TDK / DEF_SCALE_FOR_FLOAT;
	gyro_self_bias[1]
	        = st_bias[1] * DEF_GYRO_SENS_TDK / DEF_SCALE_FOR_FLOAT;
	gyro_self_bias[2]
	        = st_bias[2] * DEF_GYRO_SENS_TDK / DEF_SCALE_FOR_FLOAT;
	/* diff = bias - ZRO */
	gyro_self_diff[0]
...
struct gyroscope_sensor_operations gyro_icm42605m_ops = {
	...
	.get_gyro_power_off = get_gyro_icm42605m_power_off,
