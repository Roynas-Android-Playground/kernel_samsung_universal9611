...
int check_adc_data_spec(struct ssp_data *data, int sensortype)
{
	...
	if ((data->buf[sensortype].x == 0) &&
		...
		return FAIL;
	} else if((int)abs(data->buf[sensortype].x)
	           + (int)abs(data->buf[sensortype].y)
...
ssize_t get_magnetic_ak09918c_name(char *buf)
{
	return sprintf(buf, "%s\n", "AK09918C");
}

ssize_t get_magnetic_ak09918c_vendor(char *buf)
{
	return sprintf(buf, "%s\n", "AKM");
}

ssize_t get_magnetic_ak09918c_adc(struct ssp_data *data, char *buf)
{
	bool bSuccess = false;
	s16 sensor_buf[3] = {0, };
	int retries = 10;
...
ssize_t get_magnetic_ak09918c_dac(struct ssp_data *data, char *buf)
{
	bool bSuccess = false;
	char *buffer = NULL;
	int buffer_length = 0;
...
ssize_t get_magnetic_ak09918c_raw_data(struct ssp_data *data, char *buf)
{
...
ssize_t set_magnetic_ak09918c_raw_data(struct ssp_data *data, const char *buf)
{
	char chTempbuf[8] = { 0, };
	int ret;
	int64_t dEnable;
...
ssize_t get_magnetic_ak09918c_asa(struct ssp_data *data, char *buf)
{
	return sprintf(buf, "%d,%d,%d\n", (s16)data->uFuseRomData[0],
	               (s16)data->uFuseRomData[1], (s16)data->uFuseRomData[2]);
}

ssize_t get_magnetic_ak09918c_status(struct ssp_data *data, char *buf)
{
	bool bSuccess;
...
ssize_t get_magnetic_ak09918c_logging_data(struct ssp_data *data, char *buf)
{
	char *buffer = NULL;
	int buffer_length = 0;
	int ret = 0;
...
ssize_t get_magnetic_ak09918c_matrix(struct ssp_data *data, char *buf)
{
...
ssize_t set_magnetic_ak09918c_matrix(struct ssp_data *data, const char *buf)
{
	u8 val[PDC_SIZE] = {0, };
	int ret;
	int i;
...
ssize_t get_magnetic_ak09918c_selftest(struct ssp_data *data, char *buf)
{
	s8 result[4] = {-1, -1, -1, -1};
...
struct magnetic_sensor_operations magnetic_ak09918c_ops = {
	.get_magnetic_name = get_magnetic_ak09918c_name,
	.get_magnetic_vendor = get_magnetic_ak09918c_vendor,
	.get_magnetic_adc = get_magnetic_ak09918c_adc,
	.get_magnetic_dac = get_magnetic_ak09918c_dac,
	.get_magnetic_raw_data = get_magnetic_ak09918c_raw_data,
	.set_magnetic_raw_data = set_magnetic_ak09918c_raw_data,
	.get_magnetic_asa = get_magnetic_ak09918c_asa,
	.get_magnetic_status = get_magnetic_ak09918c_status,
	.get_magnetic_logging_data = get_magnetic_ak09918c_logging_data,
	.get_magnetic_matrix = get_magnetic_ak09918c_matrix,
	.set_magnetic_matrix = set_magnetic_ak09918c_matrix,
	.get_magnetic_selftest = get_magnetic_ak09918c_selftest,
};

struct magnetic_sensor_operations* get_magnetic_ak09918c_function_pointer(struct ssp_data *data)
{
	return &magnetic_ak09918c_ops;
}
