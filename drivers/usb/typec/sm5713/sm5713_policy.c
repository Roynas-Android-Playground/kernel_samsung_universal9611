...
static inline unsigned int CHECK_MSG_CMD(struct sm5713_usbpd_data *pd,
...
static policy_state sm5713_usbpd_policy_src_startup(
		struct sm5713_policy_data *policy)
{
	...
	if (sm5713_check_vbus_state(pd_data)) {
		if (pdic_data->reset_done == 0) {
			...
			sm5713_usbpd_init_protocol(pd_data); /* prl reset  */
		} else {
...
static policy_state sm5713_usbpd_policy_src_discovery(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_src_send_capabilities(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	struct sm5713_phydrv_data *pdic_data = pd_data->phy_driver_data;
...
static policy_state sm5713_usbpd_policy_src_negotiate_capability(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	...
	if (policy->last_state != policy->state) {
		...
		if (sm5713_usbpd_match_request(pd_data) == 0)
...
static policy_state sm5713_usbpd_policy_src_transition_supply(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_src_ready(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
#if defined(CONFIG_USB_HOST_NOTIFY)
	...
	sm5713_usbpd_set_rp_scr_sel(pd_data, PLUG_CTRL_RP180);
#endif
	...
	if ((data_role == USBPD_DFP) &&
		(sm5713_usbpd_vdm_request_enabled(pd_data))) {
...
static policy_state sm5713_usbpd_policy_src_disabled(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_src_capability_response(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_src_hard_reset(
			struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	struct sm5713_phydrv_data *pdic_data = pd_data->phy_driver_data;
...
static policy_state sm5713_usbpd_policy_src_hard_reset_received(
					struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_src_transition_to_default(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	...
	} else if (!sm5713_check_vbus_state(pd_data)) {
		msleep(tSrcRecover);
...
static policy_state sm5713_usbpd_policy_src_give_source_cap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	...
	if (policy->last_state != policy->state) {
		...
		policy->tx_data_obj[0].power_data_obj.max_current = 90;
...
static policy_state sm5713_usbpd_policy_src_give_sink_cap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int data_role = 0;
...
static policy_state sm5713_usbpd_policy_src_get_sink_cap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_src_wait_new_capabilities(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_src_send_soft_reset(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_src_soft_reset(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_snk_startup(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	struct sm5713_phydrv_data *pdic_data = pd_data->phy_driver_data;
...
static policy_state sm5713_usbpd_policy_snk_discovery(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	/* ST_PE_SNK_DISCOVERY */
...
static policy_state sm5713_usbpd_policy_snk_wait_for_capabilities(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_snk_evaluate_capability(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	...
	sink_request_obj_num =
			sm5713_usbpd_evaluate_capability(pd_data);
...
static policy_state sm5713_usbpd_policy_snk_select_capability(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	...
	if (policy->last_state != policy->state) {
		...
		policy->tx_data_obj[0] =
				sm5713_usbpd_select_capability(pd_data);
		sm5713_usbpd_send_msg(pd_data, &policy->tx_msg_header,
				policy->tx_data_obj);
	} else if (pd_data->protocol_tx.status != DEFAULT_PROTOCOL_NONE) {
		unsigned int msg;

		msg = sm5713_usbpd_wait_msg(pd_data, MSG_ACCEPT | MSG_REJECT
...
static policy_state sm5713_usbpd_policy_snk_transition_sink(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	struct sm5713_phydrv_data *pdic_data = pd_data->phy_driver_data;
...
static policy_state sm5713_usbpd_policy_snk_ready(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int data_role = 0, i = 0;
	...
	if (data_role == USBPD_DFP) {
		if (sm5713_usbpd_vdm_request_enabled(pd_data)) {
			msleep(tDiscoverIdentity);
			return PE_DFP_VDM_Identity_Request;
		}
	}
...
static policy_state sm5713_usbpd_policy_snk_hard_reset(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	struct sm5713_phydrv_data *pdic_data = pd_data->phy_driver_data;
...
static policy_state sm5713_usbpd_policy_snk_transition_to_default(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_snk_give_sink_cap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_snk_get_source_cap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_snk_send_soft_reset(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_snk_soft_reset(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_snk_give_source_cap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_drs_evaluate_port(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int data_role = 0;
...
static policy_state sm5713_usbpd_policy_drs_evaluate_send_port(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int data_role = 0;
...
static policy_state sm5713_usbpd_policy_drs_dfp_ufp_evaluate_dr_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	bool drs_ok;
...
static policy_state sm5713_usbpd_policy_drs_dfp_ufp_accept_dr_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_drs_dfp_ufp_change_to_ufp(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_drs_dfp_ufp_send_dr_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_drs_dfp_ufp_reject_dr_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_drs_ufp_dfp_evaluate_dr_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	bool drs_ok;
	...
	if (drs_ok && sm5713_check_vbus_state(pd_data))
		return PE_DRS_UFP_DFP_Accept_DR_Swap;
	else
		return PE_DRS_UFP_DFP_Reject_DR_Swap;
}

static policy_state sm5713_usbpd_policy_drs_ufp_dfp_accept_dr_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_drs_ufp_dfp_change_to_dfp(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_drs_ufp_dfp_send_dr_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_drs_ufp_dfp_reject_dr_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_prs_src_snk_reject_pr_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_prs_src_snk_evaluate_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	bool prs_ok;
...
static policy_state sm5713_usbpd_policy_prs_src_snk_send_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_prs_src_snk_accept_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_prs_src_snk_transition_to_off(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	...
	if (policy->plug_valid) {
		msleep(25);
		sm5713_usbpd_turn_off_power_supply(pd_data);

		sm5713_cc_state_hold_on_off(pd_data, 1); /* CC State Hold On */
		msleep(350);
	}
	return PE_PRS_SRC_SNK_Assert_Rd;
}

static policy_state sm5713_usbpd_policy_prs_src_snk_assert_rd(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_prs_src_snk_wait_source_on(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
#if defined(CONFIG_TYPEC)
...
static policy_state sm5713_usbpd_policy_prs_snk_src_reject_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_prs_snk_src_evaluate_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	bool prs_ok;
...
static policy_state sm5713_usbpd_policy_prs_snk_src_send_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_prs_snk_src_accept_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_prs_snk_src_transition_to_off(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	struct sm5713_usbpd_manager_data *manager = &pd_data->manager;
...
static policy_state sm5713_usbpd_policy_prs_snk_src_assert_rp(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_prs_snk_src_source_on(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
#if defined(CONFIG_TYPEC)
	...
	if (policy->last_state != policy->state) {
		sm5713_cc_state_hold_on_off(pd_data, 0); /* CC State Hold Off */
		sm5713_usbpd_turn_on_source(pd_data);
...
static policy_state sm5713_usbpd_policy_vcs_evaluate_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	bool vcs_ok;
...
static policy_state sm5713_usbpd_policy_vcs_accept_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int vconn_source = 0;
...
static policy_state sm5713_usbpd_policy_vcs_send_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int vconn_source = 0;
...
static policy_state sm5713_usbpd_policy_vcs_wait_for_vconn(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_vcs_turn_off_vconn(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_vcs_turn_on_vconn(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_vcs_send_ps_rdy(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_vcs_reject_vconn_swap(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_get_identity(
		struct sm5713_policy_data *policy)
{
...
static policy_state sm5713_usbpd_policy_ufp_vdm_send_identity(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_get_identity_nak(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_get_svids(
		struct sm5713_policy_data *policy)
{
	return PE_UFP_VDM_Get_SVIDs_NAK;
}

static policy_state sm5713_usbpd_policy_ufp_vdm_send_svids(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_get_svids_nak(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_get_modes(
		struct sm5713_policy_data *policy)
{
	return PE_UFP_VDM_Get_Modes_NAK;
}

static policy_state sm5713_usbpd_policy_ufp_vdm_send_modes(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_get_modes_nak(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_evaluate_mode_entry(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_ufp_vdm_mode_entry_ack(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_mode_entry_nak(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_mode_exit(
		struct sm5713_policy_data *policy)
{
/*
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_ufp_vdm_mode_exit_ack(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_mode_exit_nak(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_attention_request(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_evaluate_status(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_ufp_vdm_status_ack(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_status_nak(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_evaluate_configure(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_ufp_vdm_configure_ack(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_ufp_vdm_configure_nak(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_dfp_vdm_identity_request(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_policy_dfp_vdm_response(
					struct sm5713_policy_data *policy,
					sm5713_usbpd_manager_event_type event)
{
	...
	sm5713_usbpd_inform_event(pd_data, event);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_identity_acked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_identity_naked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_svids_request(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
	...
	if (policy->last_state != policy->state) {
		...
		sm5713_usbpd_send_msg(pd_data, &policy->tx_msg_header, policy->tx_data_obj);
	} else if (pd_data->protocol_tx.status != DEFAULT_PROTOCOL_NONE) {
		if (pd_data->protocol_tx.status == MESSAGE_SENT) {
...
static policy_state sm5713_usbpd_policy_dfp_vdm_svids_acked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_svids_naked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_modes_request(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	struct sm5713_usbpd_manager_data *manager = &pd_data->manager;
	...
	if (policy->last_state != policy->state) {
		...
		if (pd_data->protocol_tx.status == MESSAGE_SENT) {
			if (sm5713_usbpd_wait_msg(pd_data, VDM_DISCOVER_MODE, tVDMSenderResponse)) {
				if (policy->rx_data_obj[0].structured_vdm.command_type == Responder_ACK) {
...
static policy_state sm5713_usbpd_policy_dfp_vdm_modes_acked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_modes_naked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_entry_request(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	struct sm5713_usbpd_manager_data *manager = &pd_data->manager;
	...
	if (policy->last_state != policy->state) {
		...
		if (pd_data->protocol_tx.status == MESSAGE_SENT) {
			if (sm5713_usbpd_wait_msg(pd_data, VDM_ENTER_MODE, tVDMWaitModeEntry)) {
				if (policy->rx_data_obj[0].structured_vdm.command_type == Responder_ACK)
...
static policy_state sm5713_usbpd_policy_dfp_vdm_entry_acked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_entry_naked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_exit_request(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
	...
	if (policy->last_state != policy->state) {
		...
		if (pd_data->protocol_tx.status == MESSAGE_SENT) {
			if (sm5713_usbpd_wait_msg(pd_data, VDM_EXIT_MODE, tVDMWaitModeExit)) {
				if (policy->rx_data_obj[0].structured_vdm.command_type == Responder_ACK)
...
static policy_state sm5713_usbpd_policy_dfp_vdm_exit_acked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_exit_naked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_attention_request(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	struct sm5713_usbpd_manager_data *manager = &pd_data->manager;
...
static policy_state sm5713_usbpd_policy_dfp_vdm_status_update(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	struct sm5713_phydrv_data *pdic_data = pd_data->phy_driver_data;
					...
					if (policy->rx_data_obj[1].displayport_status.port_connected == 0x00) {
						...
						if (policy->rx_data_obj[1].displayport_status.multi_function_preferred == 1) {
							if (manager->pin_assignment & PIN_ASSIGNMENT_D)
								manager->dp_selected_pin = 4;
							else if (manager->pin_assignment & PIN_ASSIGNMENT_B)
								manager->dp_selected_pin = 2;
							...
							if (manager->pin_assignment & PIN_ASSIGNMENT_C)
								...
								manager->dp_selected_pin = 5;
							else if (manager->pin_assignment & PIN_ASSIGNMENT_A)
								manager->dp_selected_pin = 1;
...
static policy_state sm5713_usbpd_policy_dfp_vdm_status_update_acked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_status_update_naked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_displayport_configure(
	struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	struct sm5713_phydrv_data *pdic_data = pd_data->phy_driver_data;
	...
	if (policy->last_state != policy->state) {
		...
		if (pd_data->protocol_tx.status == MESSAGE_SENT) {
			if (sm5713_usbpd_wait_msg(pd_data, VDM_DP_CONFIGURE, tVDMSenderResponse)) {
				if (policy->rx_data_obj[0].structured_vdm.command_type == Responder_ACK) {
					if (manager->SVID_0 == PD_SID_1) {
						/* Notify to DP_LINK_CONFIGURE */
						sm5713_ccic_event_work(pdic_data, CCIC_NOTIFY_DEV_DP,
							CCIC_NOTIFY_ID_DP_LINK_CONF, manager->dp_selected_pin, 0, 0);
					}
					if ((policy->rx_data_obj[0].structured_vdm.svid == PD_SID_1) &&
						(manager->SVID_1 == 0x04E8)) {
						/* To do dex discover mode request */
					}
...
static policy_state sm5713_usbpd_policy_dfp_vdm_displayport_configure_acked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_vdm_displayport_configure_naked(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
...
static policy_state sm5713_usbpd_policy_dfp_uvdm_send_message(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	struct sm5713_usbpd_manager_data *manager = &pd_data->manager;
	...
	sm5713_usbpd_send_msg(pd_data, &manager->uvdm_msg_header,
...
static policy_state sm5713_usbpd_policy_dfp_uvdm_receive_message(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
void sm5713_usbpd_policy_reset(
		struct sm5713_usbpd_data *pd_data, unsigned int flag)
{
...
static policy_state sm5713_usbpd_policy_bist_carrier_m2(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);
	int power_role = 0;
...
static policy_state sm5713_usbpd_error_recovery(
		struct sm5713_policy_data *policy)
{
	struct sm5713_usbpd_data *pd_data = policy_to_usbpd(policy);

	dev_err(pd_data->dev, "%s\n", __func__);
	sm5713_error_recovery_mode(pd_data);
	return Error_Recovery;
}

void sm5713_usbpd_policy_work(struct work_struct *work)
{
	struct sm5713_usbpd_data *pd_data = container_of(work, struct sm5713_usbpd_data,
			worker);
	...
	do {
		...
		case PE_SRC_Startup:
			policy->state =
				sm5713_usbpd_policy_src_startup(policy);
			break;
		case PE_SRC_Discovery:
			policy->state =
				sm5713_usbpd_policy_src_discovery(policy);
			break;
		case PE_SRC_Send_Capabilities:
			policy->state =
				sm5713_usbpd_policy_src_send_capabilities(
					policy);
			break;
		case PE_SRC_Negotiate_Capability:
			policy->state =
				sm5713_usbpd_policy_src_negotiate_capability(
					policy);
			break;
		case PE_SRC_Transition_Supply:
			policy->state =
				sm5713_usbpd_policy_src_transition_supply(
					policy);
			break;
		case PE_SRC_Ready:
			policy->state =
				sm5713_usbpd_policy_src_ready(policy);
			break;
		case PE_SRC_Disabled:
			policy->state =
				sm5713_usbpd_policy_src_disabled(policy);
			break;
		case PE_SRC_Capability_Response:
			policy->state =
				sm5713_usbpd_policy_src_capability_response(
					policy);
			break;
		case PE_SRC_Hard_Reset:
			policy->state =
				sm5713_usbpd_policy_src_hard_reset(policy);
#if defined(CONFIG_USB_NOTIFY_PROC_LOG)
		...
		case PE_SRC_Hard_Reset_Received:
			policy->state =
				sm5713_usbpd_policy_src_hard_reset_received(
					policy);
#if defined(CONFIG_USB_NOTIFY_PROC_LOG)
		...
		case PE_SRC_Transition_to_default:
			policy->state =
				sm5713_usbpd_policy_src_transition_to_default(
					policy);
			break;
		case PE_SRC_Give_Source_Cap:
			policy->state =
				sm5713_usbpd_policy_src_give_source_cap(policy);
			break;
		case PE_SRC_Get_Sink_Cap:
			policy->state =
				sm5713_usbpd_policy_src_get_sink_cap(policy);
			break;
		case PE_SRC_Wait_New_Capabilities:
			policy->state =
				sm5713_usbpd_policy_src_wait_new_capabilities(
					policy);
			break;
		case PE_SRC_Give_Sink_Cap:
			policy->state =
				sm5713_usbpd_policy_src_give_sink_cap(policy);
			break;
		case PE_SRC_Send_Soft_Reset:
			policy->state =
				sm5713_usbpd_policy_src_send_soft_reset(policy);
			break;
		case PE_SRC_Soft_Reset:
			policy->state =
				sm5713_usbpd_policy_src_soft_reset(policy);
			break;
		case PE_SNK_Startup:
			policy->state =
				sm5713_usbpd_policy_snk_startup(policy);
			break;
		case PE_SNK_Discovery:
			policy->state =
				sm5713_usbpd_policy_snk_discovery(policy);
			break;
		case PE_SNK_Wait_for_Capabilities:
			policy->state =
				sm5713_usbpd_policy_snk_wait_for_capabilities(
					policy);
			break;
		case PE_SNK_Evaluate_Capability:
			policy->state =
				sm5713_usbpd_policy_snk_evaluate_capability(
					policy);
			break;
		case PE_SNK_Select_Capability:
			policy->state =
				sm5713_usbpd_policy_snk_select_capability(
					policy);
			break;
		case PE_SNK_Transition_Sink:
			policy->state =
				sm5713_usbpd_policy_snk_transition_sink(policy);
			break;
		case PE_SNK_Ready:
			policy->state =
				sm5713_usbpd_policy_snk_ready(policy);
			break;
		case PE_SNK_Hard_Reset:
			policy->state =
				sm5713_usbpd_policy_snk_hard_reset(policy);
			break;
		case PE_SNK_Transition_to_default:
			policy->state =
				sm5713_usbpd_policy_snk_transition_to_default(
					policy);
			break;
		case PE_SNK_Give_Sink_Cap:
			policy->state =
				sm5713_usbpd_policy_snk_give_sink_cap(policy);
			break;
		case PE_SNK_Get_Source_Cap:
			policy->state =
				sm5713_usbpd_policy_snk_get_source_cap(policy);
			break;
		case PE_SNK_Send_Soft_Reset:
			policy->state =
				sm5713_usbpd_policy_snk_send_soft_reset(policy);
			break;
		case PE_SNK_Soft_Reset:
			policy->state =
				sm5713_usbpd_policy_snk_soft_reset(policy);
			break;
		case PE_SNK_Give_Source_Cap:
			policy->state =
				sm5713_usbpd_policy_snk_give_source_cap(policy);
			break;
		case PE_DRS_Evaluate_Port:
			policy->state =
				sm5713_usbpd_policy_drs_evaluate_port(policy);
			break;
		case PE_DRS_Evaluate_Send_Port:
			policy->state =
				sm5713_usbpd_policy_drs_evaluate_send_port(
					policy);
			break;
		case PE_DRS_DFP_UFP_Evaluate_DR_Swap:
			policy->state =
				sm5713_usbpd_policy_drs_dfp_ufp_evaluate_dr_swap(
					policy);
			break;
		case PE_DRS_DFP_UFP_Accept_DR_Swap:
			policy->state =
				sm5713_usbpd_policy_drs_dfp_ufp_accept_dr_swap(
					policy);
			break;
		case PE_DRS_DFP_UFP_Change_to_UFP:
			policy->state =
				sm5713_usbpd_policy_drs_dfp_ufp_change_to_ufp(
					policy);
			break;
		case PE_DRS_DFP_UFP_Send_DR_Swap:
			policy->state =
				sm5713_usbpd_policy_drs_dfp_ufp_send_dr_swap(
					policy);
			break;
		case PE_DRS_DFP_UFP_Reject_DR_Swap:
			policy->state =
				sm5713_usbpd_policy_drs_dfp_ufp_reject_dr_swap(
					policy);
			break;
		case PE_DRS_UFP_DFP_Evaluate_DR_Swap:
			policy->state =
				sm5713_usbpd_policy_drs_ufp_dfp_evaluate_dr_swap(
					policy);
			break;
		case PE_DRS_UFP_DFP_Accept_DR_Swap:
			policy->state =
				sm5713_usbpd_policy_drs_ufp_dfp_accept_dr_swap(
					policy);
			break;
		case PE_DRS_UFP_DFP_Change_to_DFP:
			policy->state =
				sm5713_usbpd_policy_drs_ufp_dfp_change_to_dfp(
					policy);
			break;
		case PE_DRS_UFP_DFP_Send_DR_Swap:
			policy->state =
				sm5713_usbpd_policy_drs_ufp_dfp_send_dr_swap(
					policy);
			break;
		case PE_DRS_UFP_DFP_Reject_DR_Swap:
			policy->state =
				sm5713_usbpd_policy_drs_ufp_dfp_reject_dr_swap(
					policy);
			break;
		case PE_PRS_SRC_SNK_Reject_PR_Swap:
			policy->state =
				sm5713_usbpd_policy_prs_src_snk_reject_pr_swap(
					policy);
			break;
		case PE_PRS_SRC_SNK_Evaluate_Swap:
			policy->state =
				sm5713_usbpd_policy_prs_src_snk_evaluate_swap(
					policy);
			break;
		case PE_PRS_SRC_SNK_Send_Swap:
			policy->state =
				sm5713_usbpd_policy_prs_src_snk_send_swap(
					policy);
			break;
		case PE_PRS_SRC_SNK_Accept_Swap:
			policy->state =
				sm5713_usbpd_policy_prs_src_snk_accept_swap(
					policy);
			break;
		case PE_PRS_SRC_SNK_Transition_off:
			policy->state =
				sm5713_usbpd_policy_prs_src_snk_transition_to_off(
					policy);
			break;
		case PE_PRS_SRC_SNK_Assert_Rd:
			policy->state =
				sm5713_usbpd_policy_prs_src_snk_assert_rd(
					policy);
			break;
		case PE_PRS_SRC_SNK_Wait_Source_on:
			policy->state =
				sm5713_usbpd_policy_prs_src_snk_wait_source_on(
					policy);
			break;
		case PE_PRS_SNK_SRC_Reject_Swap:
			policy->state =
				sm5713_usbpd_policy_prs_snk_src_reject_swap(
					policy);
			break;
		case PE_PRS_SNK_SRC_Evaluate_Swap:
			policy->state =
				sm5713_usbpd_policy_prs_snk_src_evaluate_swap(
					policy);
			break;
		case PE_PRS_SNK_SRC_Send_Swap:
			policy->state =
				sm5713_usbpd_policy_prs_snk_src_send_swap(
					policy);
			break;
		case PE_PRS_SNK_SRC_Accept_Swap:
			policy->state =
				sm5713_usbpd_policy_prs_snk_src_accept_swap(
					policy);
			break;
		case PE_PRS_SNK_SRC_Transition_off:
			policy->state =
				sm5713_usbpd_policy_prs_snk_src_transition_to_off(
					policy);
			break;
		case PE_PRS_SNK_SRC_Assert_Rp:
			policy->state =
				sm5713_usbpd_policy_prs_snk_src_assert_rp(
					policy);
			break;
		case PE_PRS_SNK_SRC_Source_on:
			policy->state =
				sm5713_usbpd_policy_prs_snk_src_source_on(
					policy);
			break;
		case PE_VCS_Evaluate_Swap:
			policy->state =
				sm5713_usbpd_policy_vcs_evaluate_swap(policy);
			break;
		case PE_VCS_Accept_Swap:
			policy->state =
				sm5713_usbpd_policy_vcs_accept_swap(policy);
			break;
		case PE_VCS_Wait_for_VCONN:
			policy->state =
				sm5713_usbpd_policy_vcs_wait_for_vconn(policy);
			break;
		case PE_VCS_Turn_Off_VCONN:
			policy->state =
				sm5713_usbpd_policy_vcs_turn_off_vconn(policy);
			break;
		case PE_VCS_Turn_On_VCONN:
			policy->state =
				sm5713_usbpd_policy_vcs_turn_on_vconn(policy);
			break;
		case PE_VCS_Send_PS_RDY:
			policy->state =
				sm5713_usbpd_policy_vcs_send_ps_rdy(policy);
			break;
		case PE_VCS_Send_Swap:
			policy->state =
				sm5713_usbpd_policy_vcs_send_swap(policy);
			break;
		case PE_VCS_Reject_VCONN_Swap:
			policy->state =
				sm5713_usbpd_policy_vcs_reject_vconn_swap(
					policy);
			break;
		case PE_UFP_VDM_Get_Identity:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_get_identity(
					policy);
			break;
		case PE_UFP_VDM_Send_Identity:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_send_identity(
					policy);
			break;
		case PE_UFP_VDM_Get_Identity_NAK:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_get_identity_nak(
					policy);
			break;
		case PE_UFP_VDM_Get_SVIDs:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_get_svids(policy);
			break;
		case PE_UFP_VDM_Send_SVIDs:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_send_svids(policy);
			break;
		case PE_UFP_VDM_Get_SVIDs_NAK:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_get_svids_nak(
					policy);
			break;
		case PE_UFP_VDM_Get_Modes:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_get_modes(policy);
			break;
		case PE_UFP_VDM_Send_Modes:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_send_modes(policy);
			break;
		case PE_UFP_VDM_Get_Modes_NAK:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_get_modes_nak(
					policy);
			break;
		case PE_UFP_VDM_Evaluate_Mode_Entry:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_evaluate_mode_entry(
					policy);
			break;
		case PE_UFP_VDM_Mode_Entry_ACK:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_mode_entry_ack(
					policy);
			break;
		case PE_UFP_VDM_Mode_Entry_NAK:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_mode_entry_nak(
					policy);
			break;
		case PE_UFP_VDM_Mode_Exit:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_mode_exit(policy);
			break;
		case PE_UFP_VDM_Mode_Exit_ACK:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_mode_exit_ack(
					policy);
			break;
		case PE_UFP_VDM_Mode_Exit_NAK:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_mode_exit_nak(
					policy);
			break;
		case PE_UFP_VDM_Attention_Request:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_attention_request(
					policy);
			break;
		case PE_UFP_VDM_Evaluate_Status:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_evaluate_status(
					policy);
			break;
		case PE_UFP_VDM_Status_ACK:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_status_ack(policy);
			break;
		case PE_UFP_VDM_Status_NAK:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_status_nak(policy);
			break;
		case PE_UFP_VDM_Evaluate_Configure:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_evaluate_configure(
					policy);
			break;
		case PE_UFP_VDM_Configure_ACK:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_configure_ack(
					policy);
			break;
		case PE_UFP_VDM_Configure_NAK:
			policy->state =
				sm5713_usbpd_policy_ufp_vdm_configure_nak(
					policy);
			break;
		case PE_DFP_VDM_Identity_Request:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_identity_request(
					policy);
			break;
		case PE_DFP_VDM_Identity_ACKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_identity_acked(
					policy);
			break;
		case PE_DFP_VDM_Identity_NAKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_identity_naked(
					policy);
			break;
		case PE_DFP_VDM_SVIDs_Request:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_svids_request(
					policy);
			break;
		case PE_DFP_VDM_SVIDs_ACKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_svids_acked(policy);
			break;
		case PE_DFP_VDM_SVIDs_NAKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_svids_naked(policy);
			break;
		case PE_DFP_VDM_Modes_Request:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_modes_request(
					policy);
			break;
		case PE_DFP_VDM_Modes_ACKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_modes_acked(policy);
			break;
		case PE_DFP_VDM_Modes_NAKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_modes_naked(policy);
			break;
		case PE_DFP_VDM_Mode_Entry_Request:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_entry_request(
					policy);
			break;
		case PE_DFP_VDM_Mode_Entry_ACKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_entry_acked(policy);
			break;
		case PE_DFP_VDM_Mode_Entry_NAKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_entry_naked(policy);
			break;
		case PE_DFP_VDM_Mode_Exit_Request:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_exit_request(
					policy);
			break;
		case PE_DFP_VDM_Mode_Exit_ACKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_exit_acked(policy);
			break;
		case PE_DFP_VDM_Mode_Exit_NAKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_exit_naked(policy);
			break;
		case PE_DFP_VDM_Attention_Request:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_attention_request(
					policy);
			break;
		case PE_DFP_VDM_Status_Update:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_status_update(
					policy);
			break;
		case PE_DFP_VDM_Status_Update_ACKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_status_update_acked(
					policy);
			break;
		case PE_DFP_VDM_Status_Update_NAKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_status_update_naked(
					policy);
			break;
		case PE_DFP_VDM_DisplayPort_Configure:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_displayport_configure(
					policy);
			break;
		case PE_DFP_VDM_DisplayPort_Configure_ACKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_displayport_configure_acked(
					policy);
			break;
		case PE_DFP_VDM_DisplayPort_Configure_NAKed:
			policy->state =
				sm5713_usbpd_policy_dfp_vdm_displayport_configure_naked(
					policy);
			break;
		case PE_DFP_UVDM_Send_Message:
			policy->state =
				sm5713_usbpd_policy_dfp_uvdm_send_message(
					policy);
			break;
		case PE_DFP_UVDM_Receive_Message:
			policy->state =
				sm5713_usbpd_policy_dfp_uvdm_receive_message(
					policy);
			break;
		case PE_BIST_CARRIER_M2:
			policy->state = sm5713_usbpd_policy_bist_carrier_m2(policy);
			break;
		case Error_Recovery:
			policy->state = sm5713_usbpd_error_recovery(policy);
			break;
		default:
			pd_data->phy_ops.get_power_role(pd_data, &power_role);
...
void sm5713_usbpd_init_policy(struct sm5713_usbpd_data *pd_data)
{
	int i;
	struct sm5713_policy_data *policy = &pd_data->policy;
...
void sm5713_usbpd_kick_policy_work(struct device *dev)
{
