...
static void certify_cover_work(struct work_struct *work)
{
	...
	first = !gpio_get_value(ddata->pdata->gpio_certify_cover);
	...
	second = !gpio_get_value(ddata->pdata->gpio_certify_cover);
		...
		bool flip_certify_status)
{
	...
	if (flip_certify_status) {
		/* 50ms */
		wake_lock_timeout(&ddata->certify_wake_lock, HZ * 5 / 100);
		/* 10ms */
		schedule_delayed_work(&ddata->certify_cover_dwork,
				HZ * 1 / 100);
	} else {
...
static irqreturn_t certify_cover_detect(int irq, void *dev_id)
{
	...
	flip_certify_status = !gpio_get_value(ddata->pdata->gpio_certify_cover);
...
static void init_certify_hall_ic_irq(struct input_dev *input)
{
	...
	int ret = 0;
	int irq = ddata->pdata->irq_certify_cover;

	certify_cover = gpio_get_value(ddata->pdata->gpio_certify_cover);
	...
	if (ret < 0) {
		pr_err("keys: failed to certify about request flip cover irq %d gpio %d\n",
			irq, ddata->pdata->gpio_certify_cover);
	} else {
		pr_info("%s : success\n", __func__);
	}
}
...
#ifdef CONFIG_OF
static struct hall_platform_data *of_certify_hall_data_parsing_dt(
		struct device *dev)
{
	struct device_node *np_certify_hall;
	struct hall_platform_data *pdata;
	int gpio;
	enum of_gpio_flags flags;
	int ret = 0;

	np_certify_hall = dev->of_node;
	if (np_certify_hall == NULL) {
	...
	gpio = of_get_named_gpio_flags(np_certify_hall,
			"certify_hall,gpio_certify_cover", 0, &flags);
	if (gpio < 0) {
		pr_err("%s: fail to get certify_cover\n", __func__);
		ret = -EINVAL;
		goto err_out;
	}
	pdata->gpio_certify_cover = gpio;
...
#ifdef CONFIG_OF
	if (!pdata) {
		pdata = of_certify_hall_data_parsing_dt(dev);
		if (IS_ERR(pdata)) {
...
static int certify_hall_suspend(struct device *dev)
{
	...
	pr_info("%s start\n", __func__);
	status = !gpio_get_value(ddata->pdata->gpio_certify_cover);
	pr_info("[keys] %s certify_status : %d (%s)\n", __func__, status,
	 ...
	 * during freezing
	 */
	enable_irq_wake(ddata->pdata->irq_certify_cover);
...
static SIMPLE_DEV_PM_OPS(hall_pm_ops, certify_hall_suspend,
		certify_hall_resume);
