		...
		struct sec_battery_info *battery)
{
	...
	for (i = 0; i < SEC_BATTERY_CABLE_MAX; i++) {
		pr_debug("%s : CABLE_NUM(%d) INPUT(%d) CHARGING(%d)\n",
			__func__, i,
	...
	if (!np) {
		...
		for (i = 0; i < SEC_WIRELESS_RX_POWER_MAX; i++) {
			pr_debug("%s : POWER_LIST(%d) POWER_CLASS(%d) VOUT(%d) INPUT(%d) CHARGING(%d) TTF(%d) POWER(%d)\n",
				__func__, i,
	...
	pr_debug("%s : TOPOFF_1ST(%d), TOPOFF_2ND(%d)\n",
	...
	if (ret)
		pr_debug("%s : battery_full_capacity is Empty\n", __func__);
#if defined(CONFIG_BATTERY_CISD)	
	else {
		pr_debug("%s : battery_full_capacity : %d\n", __func__, pdata->battery_full_capacity);
	...
	if (ret) {
		pr_debug("%s : cisd_max_voltage_thr is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s : cisd_alg_index is Empty. Defalut set to six\n", __func__);
		pdata->cisd_alg_index = 6;
	} else {
		pr_debug("%s : set cisd_alg_index : %d\n", __func__, pdata->cisd_alg_index);
	}
#endif
	...
	if (ret) {
		pr_debug("expired time is empty\n");
	...
	if (ret) {
		pr_debug("standard_curr is empty\n");
	...
	if (ret)
		pr_debug("%s: Chip vendor is Empty\n", __func__);
	...
	if (ret)
		pr_debug("%s : battery,polling_time is Empty\n", __func__);

	/* battery thermistor */
	ret = of_property_read_u32(np, "battery,thermal_source",
		&pdata->thermal_source);
	if (ret)
		pr_debug("%s : Thermal source is Empty\n", __func__);
	...
	if (pdata->thermal_source == SEC_BATTERY_THERMAL_SOURCE_ADC) {
		...
		for(i = 0; i < pdata->temp_adc_table_size; i++) {
			...
			if (ret)
				pr_debug("%s : Temp_adc_table(adc) is Empty\n",
					__func__);
			...
			if (ret)
				pr_debug("%s : Temp_adc_table(data) is Empty\n",
					__func__);
			...
			if (ret)
				pr_debug("%s : Temp_amb_adc_table(adc) is Empty\n",
					__func__);
			...
			if (ret)
				pr_debug("%s : Temp_amb_adc_table(data) is Empty\n",
					__func__);
		}
	}
	...
	if (ret)
		pr_debug("%s : Temp check type is Empty\n", __func__);
	...
	if (ret)
		pr_debug("%s : usb_thermal_source is Empty\n", __func__);
	...
	if(pdata->usb_thermal_source == SEC_BATTERY_THERMAL_SOURCE_ADC) {
		...
		for(i = 0; i < pdata->usb_temp_adc_table_size; i++) {
			...
			if (ret)
				pr_debug("%s : Usb_Temp_adc_table(adc) is Empty\n",
					__func__);
			...
			if (ret)
				pr_debug("%s : Usb_Temp_adc_table(data) is Empty\n",
					__func__);
		}
	}
	...
	if(pdata->chg_thermal_source == SEC_BATTERY_THERMAL_SOURCE_ADC) {
		...
		for(i = 0; i < pdata->chg_temp_adc_table_size; i++) {
			...
			if (ret)
				pr_debug("%s : CHG_Temp_adc_table(adc) is Empty\n",
					__func__);
			...
			if (ret)
				pr_debug("%s : CHG_Temp_adc_table(data) is Empty\n",
					__func__);
		}
	}
	...
	if(pdata->wpc_thermal_source == SEC_BATTERY_THERMAL_SOURCE_ADC) {
		...
		if (!p) {
			...
			for(i = 0; i < pdata->wpc_temp_adc_table_size; i++) {
				...
				if (ret)
					pr_debug("%s : WPC_Temp_adc_table(adc) is Empty\n",
						__func__);
				...
				if (ret)
					pr_debug("%s : WPC_Temp_adc_table(data) is Empty\n",
						__func__);
			}
		}
	}
	...
	if(pdata->sub_bat_thermal_source) {
		...
		if (!p) {
			...
			for(i = 0; i < pdata->sub_bat_temp_adc_table_size; i++) {
				...
				if (ret)
					pr_debug("%s : sub_bat_adc_table(adc) is Empty\n",
						__func__);
				...
				if (ret)
					pr_debug("%s : sub_bat_temp_adc_table(data) is Empty\n",
						__func__);
			}
		}
	}
	...
	if (ret)
		pr_debug("%s : sub_bat_temp_check_type is Empty\n", __func__);

	/* slave thermistor */
	ret = of_property_read_u32(np, "battery,slave_thermal_source",
		&pdata->slave_thermal_source);
	if (ret)
		pr_debug("%s : slave_thermal_source is Empty\n", __func__);
	...
	if(pdata->slave_thermal_source == SEC_BATTERY_THERMAL_SOURCE_ADC) {
		...
		for(i = 0; i < pdata->slave_chg_temp_adc_table_size; i++) {
			...
			if (ret)
				pr_debug("%s : slave_chg_temp_adc_table(adc) is Empty\n",
					__func__);
			...
			if (ret)
				pr_debug("%s : slave_chg_temp_adc_table(data) is Empty\n",
					__func__);
		}
	}
	...
	if (ret)
		pr_debug("%s : slave_chg_temp_check_type is Empty\n", __func__);
	...
	if (pdata->chg_temp_check_type) {
		...
		if (ret) {
			pr_debug("%s : chg_heating_prevention_method is Empty\n", __func__);
		...
		if (ret)
			pr_debug("%s : chg_12v_high_temp is Empty\n", __func__);
		...
		if (ret)
			pr_debug("%s : chg_temp_recovery is Empty\n", __func__);
	...
	if (pdata->wpc_temp_check_type) {
		...
		if (ret) {
			pr_debug("%s : wpc_temp_control_source is Empty\n", __func__);
		...
		if (ret) {
			pr_debug("%s : wpc_temp_lcd_on_control_source is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s : wc_hero_stand_hv_cv_current is Empty\n", __func__);
	...
	if (ret)
		pr_debug("%s : sleep_mode_limit_current is Empty\n", __func__);

	ret = of_property_read_u32(np, "battery,inbat_voltage",
			&pdata->inbat_voltage);
	if (ret)
		pr_debug("%s : inbat_voltage is Empty\n", __func__);
	...
	if (pdata->inbat_voltage) {
		...
		for(i = 0; i < pdata->inbat_adc_table_size; i++) {
			...
			if (ret)
				pr_debug("%s : inbat_adc_table(adc) is Empty\n",
						__func__);
			...
			if (ret)
				pr_debug("%s : inbat_adc_table(data) is Empty\n",
						__func__);
		}
	}
	...
	if (ret) {
		pr_debug("%s : pre_afc_input_current is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s : pre_afc_work_delay is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s : pre_wc_afc_input_current is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s : pre_wc_afc_work_delay is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s : prepare_ta_delay is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s : tx_stop_capacity is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s : tx_high_recovery is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s : tx_low_threshold is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s : tx_low_recovery is Empty\n", __func__);
	...
	if (ret)
		pr_debug("%s : charging_limit_by_tx_check is Empty\n", __func__);
	...
	if(pdata->charging_limit_by_tx_check) {
		...
		if (ret) {
			pr_debug("%s : charging_limit_current_by_tx is Empty\n", __func__);
	...
	if (ret)
		pr_debug("%s : wpc_input_limit_by_tx_check is Empty\n", __func__);
	...
	if(pdata->wpc_input_limit_by_tx_check) {
		...
		if (ret) {
			pr_debug("%s : wpc_input_limit_current_by_tx is Empty\n", __func__);
			pdata->wpc_input_limit_current_by_tx =
				pdata->wpc_input_limit_current;
		}
	}	
	...
	if (ret) {
		...
		pr_debug("%s : HV Charging total time is %d\n",
	...
	if (ret) {
		...
		pr_debug("%s : Normal(WC) Charging total time is %d\n",
	...
	if (ret) {
		...
		pr_debug("%s : USB Charging total time is %d\n",
	...
	if (ret) {
		pr_debug("%s: chg_float_voltage_conv is Empty\n", __func__);
	...
	if (ret) {
		...
		pr_debug("%s: swelling wireless high temp block is %d\n",
	...
	if (ret) {
		...
		pr_debug("%s: swelling wireless high temp recovery is %d\n",
	...
	if (ret)
		pr_debug("%s: swelling low temp recovery 2nd is Empty\n", __func__);

	ret = of_property_read_u32(np, "battery,swelling_low_temp_current",
					&pdata->swelling_low_temp_current);
	if (ret) {
		pr_debug("%s: swelling_low_temp_current is Empty, Defualt value 600mA \n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_low_temp_current_2nd is Empty, set swelling_low_temp_current value \n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_low_temp_topoff is Empty, Defualt value 200mA \n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_high_temp_current is Empty, Defualt value 1300mA \n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_high_temp_topoff is Empty, Defualt value 200mA \n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_wc_high_temp_current is Empty, Defualt value 600mA \n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_wc_low_temp_current is Empty, Defualt value 600mA \n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_wc_low_temp_current_2nd is Empty, set swelling_wc_low_temp_current \n", __func__);
	...
	if (pdata->swelling_drop_float_voltage_lowtemp) {
		pr_debug("%s: swellingdrop float voltage in  low temp mode\n", __func__);

		ret = of_property_read_u32(np, "battery,swelling_low_rechg_voltage",
					   &pdata->swelling_low_rechg_voltage);
		if (ret) {
			pr_debug("%s: swelling_low_rechg_voltage is Empty, Default value 4000mV \n", __func__);
	...
	if (battery->swelling_low_temp_3rd_ctrl) {
		...
		if (ret) {
			pr_debug("%s: swelling low temp recovery 3rd is Empty\n", __func__);
		...
		if (ret) {
			pr_debug("%s: swelling_low_temp_current_3rd is Empty, set swelling_low_temp_current value \n", __func__);
		...
		if (ret) {
			pr_debug("%s: swelling_wc_low_temp_current_3rd is Empty, set swelling_wc_low_temp_current \n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling drop float voltage is Empty, Default value 4250mV \n", __func__);
		...
		pr_debug("%s : swelling drop voltage(set : %d, condition : %d)\n", __func__,
	...
	if (ret) {
		pr_debug("%s: swelling_high_rechg_voltage is Empty\n", __func__);
	...
	if (ret) {
		pr_debug("%s: swelling_low_rechg_voltage is Empty\n", __func__);
				pdata->swelling_low_rechg_voltage = 4000;
	}
	...
	if (ret) {
		pr_debug("%s: swelling_low_rechg_thr is Empty\n", __func__);
		pdata->swelling_low_rechg_thr = 150;
	}

	pr_debug("%s : SWELLING_HIGH_TEMP(%d) SWELLING_HIGH_TEMP_RECOVERY(%d)\n"
	...
	if (ret) {
		...
		pr_debug("%s: ttf_hv_12v_charge_current is Empty, Defualt value %d \n",
	...
	if (ret) {
		...
		pr_debug("%s: ttf_hv_charge_current is Empty, Defualt value %d \n",
	...
	if (ret) {
		pr_debug("%s: ttf_hv_wireless_charge_current is Empty, Defualt value 0 \n", __func__);
		pdata->ttf_hv_wireless_charge_current =
	...
	if (ret) {
		pr_debug("%s: ttf_hv_12v_wireless_charge_current is Empty, Defualt value 0 \n", __func__);
		pdata->ttf_hv_12v_wireless_charge_current =
	...
	if (ret) {
		pr_debug("%s: ttf_wireless_charge_current is Empty, Defualt value 0 \n", __func__);
		pdata->ttf_wireless_charge_current =
	...
	if (ret) {
		pr_debug("%s: ttf_dc25_charge_current is Empty, Defualt value 0 \n", __func__);
		pdata->ttf_dc25_charge_current =
			pdata->charging_current[SEC_BATTERY_CABLE_9V_TA].fast_charging_current;
	}
	...
	if (ret) {
		pr_debug("%s: ttf_dc45_charge_current is Empty, Defualt value 0 \n", __func__);
	...
	if (ret) {
		pr_debug("%s : input_current_by_siop_20 is Empty\n", __func__);
	...
	if (p) {
		...
		pr_debug("%s : battery,ignore_cisd_index is Empty\n", __func__);
	}
	...
	if (p) {
		...
		pr_debug("%s : battery,ignore_cisd_index_d is Empty\n", __func__);
	}
#endif
...
void sec_bat_parse_mode_dt(struct sec_battery_info *battery)
{
	...
	if (battery->store_mode || !battery->charging_enabled) {
		...
		if (ret) {
			pr_debug("%s : store_mode_max_input_power is Empty\n", __func__);
		...
		if (pdata->wpc_temp_check_type) {
			...
			pr_debug("%s: update store_mode - wpc high_temp(t:%d, r:%d), lcd_on_high_temp(t:%d, r:%d), curr(%d, %d)\n",
				__func__,
		...
		pr_debug("%s: update siop_hv_wireless_input_limit_current(%d)\n",
